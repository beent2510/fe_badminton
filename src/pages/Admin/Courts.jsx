...
export default function AdminCourts() {
  ...
  const [formData, setFormData] = useState({ id: null, branch_id: '', name: '', description: '', price_per_hour: '', status: 'active' });
  ...
  const handleOpenAdd = () => {
    setFormData({ id: null, branch_id: '', name: '', description: '', price_per_hour: '', status: 'active' });
    setImageFile(null);
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<Add />} onClick={handleOpenAdd} sx={{ bgcolor: '#FFD600', color: '#000', '&:hover': { bgcolor: '#FFC000' } }}>Thêm sân</Button>
      </Box>
            ...
            {courts.map(row => (
              <TableRow key={row.id} sx={{ '& td': { borderBottom: '1px solid #1e1e1e' } }}>
                ...
                <TableCell>
                  ...
                  <Typography variant="caption" color="text.secondary">{row.description?.substring(0,40)}{row.description?.length > 40 ? '...' : ''}</Typography>
                </TableCell>
                <TableCell>{row.branch?.name || branches.find(b => b.id === row.branch_id)?.name || '—'}</TableCell>
                <TableCell sx={{ color: '#FFD600', fontWeight: 600 }}>{new Intl.NumberFormat('vi-VN').format(row.price_per_hour)}đ/h</TableCell>
                <TableCell>
                  <Chip
                    label={row.status === 'active' ? 'Hoạt động' : 'Bảo trì'}
                    size="small"
                    sx={{ bgcolor: row.status === 'active' ? 'rgba(34,197,94,0.15)' : 'rgba(245,158,11,0.15)', color: row.status === 'active' ? '#22c55e' : '#f59e0b', fontWeight: 600 }}
                  />
                </TableCell>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { bgcolor: '#161616', border: '1px solid #2a2a2a' } }}>
        <DialogTitle sx={{ fontWeight: 700, borderBottom: '1px solid #2a2a2a' }}>{formData.id ? 'Sửa thông tin sân' : 'Thêm sân mới'}</DialogTitle>
        <DialogContent sx={{ pt: 3 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, mb: 3, p: 2, bgcolor: '#111', borderRadius: 2, border: '1px dashed #333' }}>
            ...
            <Box>
              <Typography variant="body2" color="text.secondary" mb={1}>Ảnh sân cầu lông</Typography>
          ...
          <TextField fullWidth label="Giá tiền/giờ (VND) *" type="number" value={formData.price_per_hour} onChange={e => setFormData({ ...formData, price_per_hour: e.target.value })} margin="dense" />
        ...
        <DialogActions sx={{ p: 2, borderTop: '1px solid #2a2a2a' }}>
          ...
          <Button onClick={handleSubmit} variant="contained" sx={{ bgcolor: '#FFD600', color: '#000', '&:hover': { bgcolor: '#FFC000' }, px: 3 }}>Lưu</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
